...
#include<string.h>
#include<conio.h>//Necessária pro getch
...
void inputData(FILE *arquivo){
	fclose(arquivo);	
	char titulo[]={"NOME DA ATRAÇÃO TURÍSTICA"};
	...
	strcpy(titulo,"RESUMO DA ATRAÇÃO");
	Cabecalho(titulo);
...
///////////////////////AVALIAÇÕES///////////////////////////
void Avaliacao(struct descricao *ponteiro,FILE *arquivo){
	char avaliacoes[][20]={"Ótimo","Bom","Regular","Ruim","Péssimo","Caro","Barato","Grátis"};
	char titulo[]={"AVALIAÇÃO DE ATRAÇÃO"};
	int opcao;
	do{
		Cabecalho(titulo);
...
void SelecaoEstados(struct descricao *ponteiro){
	...
	if(comparacao==0){
		char estados[][20]={"Acre","Amapá","Amazonas","Pará","Rondônia","Roraima","Tocantins"};
		RecepcaoEstados(estados,ponteiro);
	}else{
		comparacao=strcmp(ponteiro->local.regiao,"Nordeste");
		if(comparacao==0){
			char estados[11][20]={"Alagoas","Bahia","Ceará","Maranhão","Paraíba","Pernambuco","Piauí","Rio Grande do Norte","Sergipe"};
			RecepcaoEstados(estados,ponteiro);
		}else{
			comparacao=strcmp(ponteiro->local.regiao,"Centro-Oeste");
			if(comparacao==0){
				char estados[][20]={"Goiás","Mato Grosso","Mato Grosso do Sul"};
				RecepcaoEstados(estados,ponteiro);
			}else{
				comparacao=strcmp(ponteiro->local.regiao,"Sudeste");
				if(comparacao==0){
					char estados[][20]={"Espirito Santo","Minas Gerais","Rio de Janeiro","São Paulo"}; 
					RecepcaoEstados(estados,ponteiro);
				}else{
					char estados[][20]={"Paraná","Santa Catarina","Rio Grande do Sul"};
					RecepcaoEstados(estados,ponteiro);
				}
			}
		}
...
//Menu seleção de Estados
void RecepcaoEstados(char estado[][20],struct descricao *ponteiro){
	int tamanho;
	int opcao=0;
	char titulo[]={"SELEÇÃO ESTADUAL"};
	do{
		tamanho=0;		
		Cabecalho(titulo);
...
void menuPrincipal(FILE *arquivo){
	int opcao;
	char titulo[]={"ATRAÇÕES TURÍSTICAS DO BRASIL"};
	do{
		Cabecalho(titulo);
		printf("\n1)Consultar os dados de uma região");	//OK
		printf("\n2)Incluir uma descrição");	//OK
		printf("\n3)Alterar uma descrição");	//OK
		printf("\n4)Excluir uma descrição");	//OK
		printf("\n5)Mostrar uma descrição");	//OK
		printf("\n6)Sair");
		printf("\n\nDigite a opção desejada: ");
		scanf("%d",&opcao);
	}while(opcao<1 || opcao>6);	
	switch(opcao){
		case 1:
			...
			break;
		case 2:
			inputData(arquivo); //Adiciona uma nova atração
			break;
		case 3:
		...
		case 4:
			...
			break;
		case 5:
			PesquisarPorNome(arquivo); //Mostra uma atração -> botar no mesmo bola de exclusao e edição
			break;
		case 6:
			exit(0);
			break;
...
void PesquisarPorNome(FILE *arquivo){
	...
	if(arquivo==NULL){
		...
		Cabecalho(titulo);
		fflush(stdin);
		printf("Digite o nome da atração desejada: ");
		gets(nome);
...
void AlterarDescricao(int num,FILE *arquivo){
	...
	if(arquivo==NULL){
		...
		if(arquivoSubstituto==NULL){
			...
			fflush(stdin);
			if(num==1){
				printf("Digite o nome da atração que você deseja excluir: ");	
			}else{
				printf("Digite o nome da atração que você deseja editar: ");
			}
			gets(nome);
			printf("\nResultado da Pesquisa:\n\n");
			int contador=0;
