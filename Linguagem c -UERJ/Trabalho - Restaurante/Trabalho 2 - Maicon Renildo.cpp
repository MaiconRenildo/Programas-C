...
//Protótipos das funções para os produtos
void areaProdutos();
...
//Protótipos das das funções usadas em mesas e produtos
...
void areaProdutos(){
	...
	if(produtos==NULL){	//Se o arquivo não existir, cria
...
void areaGarcons(){
	...
	if(garcons==NULL){	//Se o arquivo não existir, cria
...
void menuGarcons(FILE *garcons){
	...
	do{
		...
		printf("\n2)Deletar dados de um garçom");	//informa o codigo do produto e deleta
		printf("\n3)Alterar dados de um garçom");	//informa o codigo do produto e altera
		printf("\n4)Consultar dados de um garçom"); //informa o nome do produto e aparece as informações sobre ele
		printf("\n5)Mostrar todos os garçons");
...
void menuProdutos(FILE *produtos){
	...
	do{
		...
		printf("\n4)Consultar produto"); //informa o nome do produto e aparece as informações sobre ele
...
void cadastrarGarcom(FILE *garcons){
	...
	if(garcons==NULL){
		...
		printf("\nInforme o nome do garçom: ");
		gets(novoGarcom.nome);
		fflush(stdin);
		
		printf("\nInforme o código do garçom: ");
...
void consultarGarcons(FILE *garcons){
		...
		while( fread(&garcom,sizeof(Garcons),1,garcons)==1){
			if(strcmp(garcom.nome,nome)==0){
				contador++;
				printf("\nNome do garçom: %s",garcom.nome);
				printf("\nCódigo do garçom: %s",garcom.codigo);
			}
		}
		if(contador==0){
...
void mostrarProdutos(FILE *produtos){
	...
	while( fread(&prato,sizeof(pratosEbebidas),1,produtos)==1){
		...
		printf("\nCódigo do prato: %s",prato.codigo);
		printf("\nPreço do prato: R$ %.2f",prato.preco);
	}
	if(contador==0){
...
void consultarRegistros(FILE *produtos){
		...
		while( fread(&produto,sizeof(pratosEbebidas),1,produtos)==1){
			if(strcmp(produto.nome,nome)==0){
				...
				printf("\nCódigo do produto: %s",produto.codigo);
				printf("\nPreço do produto %.2f R$",produto.preco);
			}
		}
		if(contador==0){
...
void alterarGarcom(int num,FILE *garcons){
	...
	if(garcomsubstituto==NULL){
			printf("\nNão será possível fazer alterações. Tente novamente mais tarde.\n");
			getchar();
			fflush(stdin);
			fclose(garcons);
			exit(0);
...
void imprimirMesa(NoMesa* Principal){
	...
	if(corredor==NULL){
		...
		if(corredor==NULL){
			...
			aux=corredor->pedidos;
			printf("\nMESA %d",mesa);
			printf("\nCódigo do garçom: %s",corredor->codigo_garcom);
			printf("\nPedidos: ");
			while(aux!=NULL){
				printf("\nCodigo do pedido: %s",aux->codigoPedido);
				printf("   -   Quantidade do pedido: %d",aux->quantidade);
...
void imprimir_conta(NoMesa* mesa){
	...
	printf("\nMESA %d",mesa->numMesa);
	printf("\nCodigo do Garçom: %s",mesa->codigo_garcom);
...
void fecharConta(NoMesa *Lista){
	...
	if(existe==0){
		...
		printf("\nNão existe conta aberta nesta mesa!");
		getchar();
		fflush(stdin);
		MenuPedidos(Lista);
		...
		if(Lista->numMesa==mesa){	//verifica se é a primeira mesa
		
			imprimir_conta(Lista);			
			if(Lista->prox==NULL){	//verifica se é a primeira e unica mesa
				
				Lista=NULL;
				MenuPedidos(Lista);
				
			}else{	//é a primeira mas não a unica
		...
		}else if(corredor->numMesa==mesa){	//Verifica se é a ultima mesa
		...
		}else{ //Não é a primeira nem a ultima mesa
	...
	if(achou==0){
		...
		if(num==1){
			...
			if(strcmp(corredor->codigoPedido,codigo)!=0){	//Verifica se existe o pedido na mesa
				...
				if(strcmp(primeiro->codigoPedido,codigo)==0){	//Verifica se é o primeiro pedido
					NoPedido *substituto;
					substituto=aux->pedidos;
					if(substituto->prox==NULL){	//verifica se há somente um pedido-> exclui a mesa
						
						if(aux->ant==NULL){		//Verifica se é a primeira mesa da lista
						
							if(aux->prox==NULL){	//verifica se é a unica mesa da lista-> zera a lista
								free(aux);								
								Lista=NULL;
								getchar();
								fflush(stdin);
						...
						}else if(aux->prox==NULL){	//verifica se é a ultima
						...
						}else{		//se não é a primeira e nem a ultima, ta no meio
					...
					}else{	//Há mais de um pedido-> exclui o pedido
				...
				}else if(strcmp(ultimo->codigoPedido,codigo)==0){	//verifica se é o ultimo pedido
				...
				}else{	//Não é o primeiro nem o ultimo pedido
...
void manualPrograma(){
	...
	printf("\n----------------------------------------------");
	printf("\nMENU PRINCIPAL: Permite movimentações entre os menus de Iventário, Funcionários, Pedidos e Cardápio");
	printf("\n\nCARDÁPIO: Exibe todos os itens cadastrados");
	printf("\n\nIVENTÁRIO: Permite ao usuário inserir, consultar, alterar e excluir itens. Esses itens que são mostrados no Cardápio");
	printf("\n\nFUNCIONÁRIOS: Permite ao usário inserir, consultar, alterar e excluir informações sobre os garçons");
	printf("\n\nPEDIDOS: Neste menu é possível visualizar pedidos de uma mesa específica, visualizar todos os pedidos cadastrados e fechar a conta de uma mesa.");
	printf("Além disso, através deste menu é possivel inserir, alterar e excluir pedidos(Caso só exista um pedido na mesa, ela será removida).No entanto, há algumas condições:");
	printf("\nPara inserir um pedido é necessário que o item desejado esteja cadastrado(exista no cardápio) e que o garçom esteja cadastrado.");
	printf("\nPara alterar ou excluir um pedido é necessário ter conhecimento do número da mesa e do código do pedido.");
	getchar();
	fflush(stdin);
}
